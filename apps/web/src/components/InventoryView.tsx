...
import { Search, Plus, Barcode, AlertTriangle, ArrowUpDown, Filter, Package, Check, RefreshCw } from 'lucide-react';
...
export const InventoryView: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'cat_atta', name: 'Atta & Rice (???-????)' },
    { id: 'cat_oils', name: 'Edible Oils (???-??)' },
    { id: 'cat_spices', name: 'Spices (?????)' },
    { id: 'cat_dals', name: 'Pulses (?????)' },
    { id: 'cat_dairy', name: 'Dairy & Bread (???-?????)' },
    { id: 'cat_snacks', name: 'Snacks & Biscuits (?????)' },
    { id: 'cat_beverages', name: 'Tea & Coffee (???-????)' },
        ...
        <div className="flex items-center space-x-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search product by name, Hindi, brand, or barcode... (???. ???, ???, Parle-G)"
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
            ...
            <form onSubmit={handleAdjustSubmit} className="space-y-3">
              ...
              <div>
                ...
                >
                  <option value="Fresh Delivery Restock">?? Fresh Delivery Restock (??? ??? ???)</option>
                  <option value="Damaged / Wastage">?? Damaged / Wastage (???? / ?????????)</option>
                  <option value="Manual Physical Count Correction">?? Physical Count Correction (????? ?????)</option>
