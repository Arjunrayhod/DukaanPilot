    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 px-4 py-3 shadow-sm">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-green-700 flex items-center justify-center text-white font-bold text-lg shadow-sm">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 text-xs text-slate-500">
              <span className={`inline-block w-2 h-2 rounded-full ${isOnline ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="bg-slate-100 p-1 rounded-lg flex items-center text-xs font-semibold text-slate-700">
            <button
              onClick={() => onToggleView('mobile')}
              className={`px-2.5 py-1 rounded-md transition-colors ${
                activeView === 'mobile' ? 'bg-white text-green-800 shadow-sm' : 'hover:text-slate-900'
              }`}
            >
              ?? Mobile App
            </button>
            <button
              ...
              className={`px-2.5 py-1 rounded-md transition-colors ${
                activeView === 'pos' ? 'bg-white text-green-800 shadow-sm' : 'hover:text-slate-900'
              }`}
            >
              ?? Counter POS
            </button>
          ...
          <button
            ...
            className="flex items-center px-2.5 py-1.5 rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
          >
